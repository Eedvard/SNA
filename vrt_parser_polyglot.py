...
# blob = """Suomi perkele kuuluu natoon venäkä on vaarallinen ja tarha halonen vittu ja timo jutila"""
# text = Text(blob,hint_language_code='fi')
# print(text.entities)
# input()
...
for path in name_maker():
            ...
            for word in comment:
                ...
                for tag in tags:
                    if tag==word:
                        ...
                        # blob = """Suomi perkele kuuluu natoon venäkä on vaarallinen ja tarha halonen vittu ja timo jutila"""
                        text = Text(word,hint_language_code='fi')
                        # print(text.entities)
                        for entity in text.entities:
                            vitun_tavallinen_sana = entity[0].__repr__()
                            #print(vitun_tavallinen_sana)
                            #print(type(vitun_tavallinen_sana))
                            sub_graph_names.append(vitun_tavallinen_sana)
    ...
    if (COUNTER%10 == 0) or (COUNTER==1954):
        ...
        nx.gpickle.write_gpickle(graph, "graph_pickle"+str(COUNTER)+"polyglot")
